...
// Figure checks only care about a bare prose number (e.g. "5 languages").
// A decimal like Tailwind's `py-2.5` or an rgba alpha `0.5` has a digit
// immediately adjacent to a period, which `word()` doesn't exclude -- so
// CSS values were tripping the co-occurrence check meant for real claims.
const wordFigure = s => new RegExp(`(?<![\\w.-])${esc(s)}(?![\\w.-])`, 'gi');
...
for (const file of files) {
  ...
  for (const [key, entry] of Object.entries(proof)) {
    ...
    for (const m of text.matchAll(wordFigure(v))) {
